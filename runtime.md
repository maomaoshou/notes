...
```objective-c
如果在category中实现了load方法，那么category和类本身的load方法都会被调用，调用顺序为先调用类的load方法，再按照按照编译顺序调用多个分类的load方法。load方法是直接取内存地址进行调用的，不会走消息发送流程。initialize方法在第一次向类发送消息时调用，会走消息转发流程
```
...
* [A isKindOfClass:B]执行过程：
  ...
  取A的`meta class`和B比较，若不同，则取meta class的super class和B比较，依次循环(**根类NSObject的元类的super class为NSObject本身**)
...
**Method Swizzling模版**
  ...
  1. ![](/Users/maomaoshou/Documents/notes/assets/load&initialize.png)
   ...
   1. dispath_once,应该保证method swizzling方法只被调用一次,否则进行偶数次交换和未交换一个效果
   ...
   1. didAddMethod参数存在的意义:`class_getInstanceMethod`方法会沿着继承链寻找方法实现，如果子类没有实现方法，得到的是父类的实现，此时直接调用`class_exchageImplementations`方法后父类的方法实现会被交换，如果其它子类没有实现该方法，调用原方法会去调用父类的方法，而父类方法的实现已经被替换，会去调用swizzle的方法从而引发unrecognizer崩溃。例如
